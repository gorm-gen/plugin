...
// MultiList 获取多表数据列表
func ({{.Abbr}} *{{.StructName}}) MultiList(sharding []{{.ShardingKeyType}}) *_multiList {
	return &_multiList{
		core:          {{.Abbr}},
		unscoped:      {{.Abbr}}.unscoped,
		selects:       make([]field.Expr, 0),
...
// Do 执行获取多表数据列表
func (l *_multiList) Do(ctx context.Context) ([]*{{.ModelName}}.{{.StructName}}, int64, error) {
	...
	for k, v := range slList {
		...
		go func(k int, v list.Sl) {
			defer func() {
				if r := recover(); r != nil {
					l.core.logger.Error(fmt.Sprintf("【{{.StructName}}.MultiList.%s】执行异常-1", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
					errChan {{.ChanSign}} fmt.Errorf("recovered from panic: %v", r)
				}
			}()
			defer func() {
			...
			for kk, vv := range v.List {
				...
				go func(kk int, vv list.Sld) {
					defer func() {
						if r := recover(); r != nil {
							l.core.logger.Error(fmt.Sprintf("【{{.StructName}}.MultiList.%s】执行异常-2", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
					...
					if res, err = lr.Scopes(page.Paginate(vv.Page, vv.PageSize)).Find(); err != nil {
						if {{.RepoPkgName}}.IsRealErr(err) {
							l.core.logger.Error(fmt.Sprintf("【{{.StructName}}.MultiList.%s】失败", v.ShardingValue), zap.Error(err))
						}
